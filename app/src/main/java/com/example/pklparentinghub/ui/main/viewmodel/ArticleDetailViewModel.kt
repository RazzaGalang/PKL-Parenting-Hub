...
class ArticleDetailViewModel (private val articleDetailRepository: ArticleDetailRepository) : ViewModel() {

    fun getArticleDetail(token: String, articleId: Int) = liveData(Dispatchers.IO) {
        emit(Resource.loading(data = null))
        try {
            emit(Resource.success(data = articleDetailRepository.getArticleDetail(token, articleId)))
